...
class PluginManager {
  ...
  invoke(commandsArray, allowEntryPoints, isSpawned) {
    ...
    this.serverless.cli.log(`Invoke ${isSpawned ? '(spawned)' : ''} ${_.join(commandsArray, ':')}`);
    if (process.env.SLS_DEBUG) {
      if (hooks.length === 0) {
    ...
    events.forEach(eventName => {
      ...
      for (const hook of eventHooks) {
        hook._eventName = eventName;
        hook._id = String(++counter).padStart(2, '0');
      }
      if (eventHooks.length) {
        console.log(
          '+',
          `x${String(eventHooks.length).padStart(2, ' ')} ${eventName}, hooks: ${eventHooks
            .map(hook => hook._id)
            .join(', ')}`
        );
      } else console.log(chalk.gray('-', '   ', eventName));
    });
    console.log('');
    ...
    return BbPromise.reduce(
      ...
      (__, hook) => {
        ...
        let otherHooksTime = 0;
        console.log(chalk.yellow('Hook started:'), `${hook._id}.${hook._eventName}`);
        const listener = time => (otherHooksTime += time);
        timerEmitter.on('hooktime', listener);
        return BbPromise.resolve(hook.hook()).finally(() => {
          const time = Date.now() - start - otherHooksTime;
          timerEmitter.removeListener('hooktime', listener);
          timerEmitter.emit('hooktime', time);
          console.log(
            chalk.yellow('Hook ended  :'),
            `${hook._id}.${hook._eventName}`,
            chalk.grey(`(duration: ${(time / 1000).toFixed(2)}s)`)
          );
        });
      },
      null
  ...
  spawn(commandsArray, options) {
    ...
    return this.invoke(commands, true, true).then(() => {
